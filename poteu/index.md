**Privacy Policy**

**Last Updated:** August 18, 2025

**1. Introduction**
This Privacy Policy outlines the practices for applications developed by **b8o** (the "Developer"). This policy applies to all applications published by the Developer, including but not limited to "ПОТЭЭ-903н-2022", "782н", "ФЗ-116: Промбез офлайн"," ППР 1479: Правила пож.режима", and "ПТЭЭП 2025 • Электроустановки" (collectively, the "App" or "Apps"). The Developer is committed to protecting your privacy.

**2. Disclaimer and Source of Information**
It is crucial to understand the following:
*   **This App Does Not Represent a Government Entity.** It has no affiliation with any government body of the Russian Federation or any other country and is not authorized to facilitate government services.
*   **This is Not an Official Source.** The information provided within each App is based on publicly available regulations but is not an official publication and may contain inaccuracies or outdated content.
*   **Educational Purpose Only.** The Apps are designed solely as supplementary tools for studying the relevant regulations and standards. For legally binding purposes, always refer to official sources and publications.

**3. Data Collection and Usage**

**3.1. Data Stored Locally on Your Device**
The App stores certain data **exclusively on your device's internal storage** to save your preferences and progress. This includes settings like your chosen theme, font size, bookmarks, or notes. This data is never transmitted to the Developer or any third party and is fully under your control.

**3.2. Unique User Identifier**
To provide and manage paid subscriptions, the App generates and stores a **unique, anonymous user identifier**.
*   **Purpose:** This identifier is used to associate your subscription with your device and to verify its status with our server.
*   **Privacy:** The identifier is anonymous and is not linked to your personal information (such as name, email, etc.). It is transmitted to our server to validate your subscription status.

**3.3. Paid Services and Payment Processing**
Our Apps offer paid subscriptions that provide access to additional features and content.
*   **Important:** All payments are processed through an external, trusted third-party payment provider Tinkoff. To make a payment, you will be redirected to the secure web page of this provider.
*   **Security:** Our App **does not collect, process, or store** your credit card details or other financial information. We only receive a confirmation of a successful or failed payment from the payment gateway, which is linked to your unique user identifier.

**3.4. Third-Party Services: Crash Reporting with Sentry**
To improve the stability and performance of our Apps, we use a third-party service for error and crash monitoring called **Sentry** (provided by Functional Software, Inc.).
*   **Purpose:** Sentry helps us diagnose and fix technical problems. When the App crashes or encounters an error, Sentry automatically collects diagnostic data.
*   **Data Collected:** The information sent to Sentry is strictly for technical diagnostics and may include crash logs and device information (such as model and OS version). We do not use this information to personally identify you.
*   For more details, you can review **Sentry's Privacy Policy**: [https://sentry.io/privacy/](https://sentry.io/privacy/)

**4. Your Rights and Data Control**
You have full control over the data stored locally on your device. You can delete this data at any time by:
1.  **Clearing App Data:** Go to your Android "Settings," navigate to "Apps," select the specific application, and use the "Clear Cache" and "Clear Data" options.
2.  **Uninstalling the App:** Uninstalling the application will permanently delete all of its associated data.

**5. Security**
We are committed to ensuring the app is secure. If you discover any security vulnerability, please contact me via email.

**6. Changes to This Privacy Policy**
We may update our Privacy Policy from time to time. We will notify you of any changes by posting the new Policy on this page. You are advised to review this page periodically.

**7. Developer and Contact Information**
If you have any questions or suggestions about this Privacy Policy, do not hesitate to contact me.


**Политика конфиденциальности**

**Дата последнего обновления:** 18 августа 2025 г.

**1. Введение**
Настоящая Политика конфиденциальности описывает практики для приложений, разработанных **b8o** («Разработчик»). Политика распространяется на все приложения, опубликованные Разработчиком, включая, но не ограничиваясь: «ПОТЭЭ-903н-2022», «782н», «ФЗ-116: Промбез офлайн», «ППР 1479: Правила пож.режима» и «ПТЭЭП 2025 • Электроустановки» (совместно именуемые «Приложение» или «Приложения»). Разработчик обязуется защищать вашу конфиденциальность.

**2. Отказ от ответственности и источник информации**
Крайне важно понимать следующее:
*   **Приложения не представляют государственный орган.** Они не имеют никакого отношения к государственным органам Российской Федерации или любой другой страны и не уполномочены оказывать государственные услуги.
*   **Это не официальный источник.** Информация, представленная в каждом Приложении, основана на общедоступных нормативных актах, но не является официальной публикацией и может содержать неточности или устаревшие данные.
*   **Только в образовательных целях.** Приложения предназначены исключительно как вспомогательные инструменты для изучения соответствующих правил и стандартов. Для юридически значимых целей всегда обращайтесь к официальным источникам.

**3. Сбор и использование данных**

**3.1. Данные, хранимые локально на вашем устройстве**
Приложение хранит определенные данные **исключительно в памяти вашего устройства** для сохранения ваших предпочтений и прогресса. Это включает настройки, такие как выбранная тема, размер шрифта, закладки или заметки. Эти данные никогда не передаются Разработчику или третьим лицам и находятся под вашим полным контролем.

**3.2. Уникальный идентификатор пользователя**
Для предоставления и управления платными подписками Приложение генерирует и хранит **уникальный анонимный идентификатор пользователя**.
*   **Цель:** Этот идентификатор используется для привязки вашей подписки к вашему устройству и проверки ее статуса на нашем сервере.
*   **Конфиденциальность:** Идентификатор является анонимным и не связан с вашими личными данными (имя, email и т.д.). Он передается на наш сервер для проверки статуса подписки.

**3.3. Платные услуги и обработка платежей**
Наши Приложения предлагают платные подписки, которые предоставляют доступ к дополнительным функциям и контенту.
*   **Важно:** Все платежи обрабатываются через внешнего, надежного поставщика платежных услуг Tinkoff. Для совершения платежа вы будете перенаправлены на защищенную веб-страницу этого поставщика.
*   **Безопасность:** Наше Приложение **не собирает, не обрабатывает и не хранит** данные вашей банковской карты или другую финансовую информацию. Мы получаем от платежной системы только подтверждение об успешном или неуспешном платеже, привязанное к вашему уникальному идентификатору пользователя.

**3.4. Сторонние сервисы: Отчеты о сбоях (Sentry)**
Для улучшения стабильности и производительности наших Приложений мы используем сторонний сервис для мониторинга ошибок и сбоев под названием **Sentry** (предоставляется Functional Software, Inc.).
*   **Цель:** Sentry помогает нам диагностировать и устранять технические проблемы. Когда Приложение аварийно завершает работу или сталкивается с ошибкой, Sentry автоматически собирает диагностические данные.
*   **Собираемые данные:** Информация, отправляемая в Sentry, предназначена исключительно для технической диагностики и может включать отчеты о сбоях, информацию об устройстве (модель, версия ОС) и другие технические данные. Мы не используем эту информацию для вашей личной идентификации.
*   Для получения дополнительной информации вы можете ознакомиться с **Политикой конфиденциальности Sentry**: [https://sentry.io/privacy/](https://sentry.io/privacy/)

**4. Ваши права и контроль над данными**
Вы имеете полный контроль над данными, хранящимися локально на вашем устройстве. Вы можете удалить их в любой момент:
1.  **Очистив данные приложения:** В «Настройках» вашего устройства Android найдите нужное Приложение и используйте опции «Очистить кэш» и «Очистить данные».
2.  **Удалив приложение:** Удаление Приложения с вашего устройства полностью сотрет все связанные с ним данные.

**5. Безопасность**
Мы стремимся обеспечить безопасность Приложения. Если вы обнаружите уязвимость в безопасности, пожалуйста, свяжитесь со мной по электронной почте.

**6. Изменения в Политике конфиденциальности**
Мы можем время от времени обновлять нашу Политику конфиденциальности. Мы сообщим вам о любых изменениях, опубликовав новую Политику на этой странице. Рекомендуется периодически просматривать эту страницу.

**7. Разработчик и контактная информация**
Если у вас есть вопросы или предложения по поводу этой Политики конфиденциальности, пожалуйста, свяжитесь со мной.

**Разработчик в Google Play:** **b8o**
**Контактное лицо:** Богаченко Иван
Санкт-Петербург, Россия
Email: bogachencoivan@gmail.com
---